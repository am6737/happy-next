...
> **Note:** LiveKit enforces a minimum 32-character length for both `LIVEKIT_API_KEY` and `LIVEKIT_API_SECRET`.

| Variable | Default | Description |
|----------|---------|-------------|
| `LIVEKIT_URL` | — | LiveKit server URL (required) |
| `LIVEKIT_API_KEY` | — | LiveKit API key (required, min 32 chars) |
