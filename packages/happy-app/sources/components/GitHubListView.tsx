...
import { useGithubRepos, useGithubIssues, useGithubPulls, useGithubWorkIssues, useGithubWorkPulls, clearGithubCache } from '@/hooks/useGithubData';
...
export const GitHubListView = React.memo(({ onRepoChange, repoPickerTriggerRef }: GitHubListViewProps) => {
    const { theme } = useUnistyles();
    ...
    const [issueScope, setIssueScope] = React.useState<GithubIssueScope>('all');
    const [pullScope, setPullScope] = React.useState<GithubPullScope>('all');
    ...
    const isGlobal = selectedRepo === null;
    ...
    const repoIssues = useGithubIssues(
        owner, repoName, getIssueApiState(issueFilter), activeTab === 'issues' && !isGlobal
    );

    const repoPulls = useGithubPulls(
        owner, repoName, getPRApiState(prFilter), activeTab === 'pulls' && !isGlobal
    );

    const workIssues = useGithubWorkIssues({
        state: getIssueApiState(issueFilter),
        scope: issueScope,
        search: debouncedSearchQuery || undefined,
        enabled: isGlobal,
    });

    const workPulls = useGithubWorkPulls({
        state: prFilter,
        scope: pullScope,
        search: debouncedSearchQuery || undefined,
        enabled: isGlobal,
    ...
    const issueResult = isGlobal ? workIssues : repoIssues;
    const pullResult = isGlobal ? workPulls : repoPulls;
    const issues = issueResult.data;
    const pulls = pullResult.data;

    const filteredPulls = React.useMemo(() => {
        if (isGlobal || prFilter !== 'merged') return pulls;
        return pulls.filter((p) => p.status === 'merged');
    }, [isGlobal, pulls, prFilter]);
    ...
    const handleIssuePress = React.useCallback((item: RepoIssue) => {
        ...
        const [itemOwner, itemRepo] = repository.split('/');
        if (itemOwner && itemRepo) router.push(`/repos/${itemOwner}/${itemRepo}/issue/${item.number}`);
    }, [router, selectedRepo]);
    ...
    const handlePRPress = React.useCallback((item: RepoPR) => {
        ...
        if (!repository) return;
        const [itemOwner, itemRepo] = repository.split('/');
        if (itemOwner && itemRepo) router.push(`/repos/${itemOwner}/${itemRepo}/pulls/${item.number}`);
    }, [router, selectedRepo]);
    ...
    const handleSelectRepo = React.useCallback((repo: RepoInfo | null) => {
        setSelectedRepo(repo?.fullName ?? null);
        setSearchQuery('');
    ...
    const activeResult = activeTab === 'issues' ? issueResult : pullResult;

    const renderEmptyState = () => {
        if (isPullRefreshing) return null;
        if (isLoading) return <ActivityIndicator size="small" color={theme.colors.textSecondary} style={{ marginTop: 40 }} />;
        if (activeResult.error) {
            return (
                <View style={styles.emptyContainer}>
                    <Ionicons name="cloud-offline-outline" size={40} color={theme.colors.status.error} />
                    <Text style={[styles.emptyText, { color: theme.colors.textSecondary }]}>
                        {t('errors.serverError')}
                    </Text>
                    <Pressable
                        style={[styles.retryButton, { backgroundColor: theme.colors.text }]}
                        onPress={activeResult.refresh}
                    >
                        <Ionicons name="refresh" size={15} color={theme.colors.groupped.background} />
                        <Text style={[styles.retryButtonText, { color: theme.colors.groupped.background }]}>
                            {t('common.retry')}
                        </Text>
                    </Pressable>
                </View>
        ...
        return (
            <View style={styles.emptyContainer}>
                <Ionicons
                    name={activeTab === 'issues' ? 'alert-circle-outline' : 'git-pull-request-outline'}
                    size={40}
    ...
    const otherTabCount = isGlobal
        ...
        : (activeTab === 'issues' ? selectedRepoInfo?.openPRsCount : selectedRepoInfo?.openIssuesCount);
    ...
    const listHeader = (
        <View style={styles.listHeader}>
            <View style={styles.headlineRow}>
                <View style={styles.headlineLabelGroup}>
                    <Text
                    ...
                    <Text
                        style={[styles.headlineCount, { color: theme.colors.textSecondary }]}
                        numberOfLines={1}
                        adjustsFontSizeToFit
                    >
                        {typeof currentCount === 'number' ? currentCount : ''}
                    </Text>
                </View>
                <Pressable
                    style={[styles.swapPill, { backgroundColor: theme.colors.surface }]}
                    onPress={() => setActiveTab(otherTab)}
                >
                    <Text style={[styles.swapPillText, { color: theme.colors.textSecondary }]} numberOfLines={1} adjustsFontSizeToFit>
                        {otherTabLabel}
                    </Text>
                    <View style={[styles.swapPillBadge, { backgroundColor: theme.colors.divider }]}>
                        {otherTabCountLoading && typeof otherTabCount !== 'number' ? (
                            <ActivityIndicator
                                ...
                                color={theme.colors.textSecondary}
                                style={Platform.OS === 'ios' ? { transform: [{ scale: 0.6 }] } : undefined}
                            />
                        ) : (
                            <Text style={[styles.swapPillBadgeText, { color: theme.colors.textSecondary }]} numberOfLines={1} adjustsFontSizeToFit>
                                {typeof otherTabCount === 'number' ? otherTabCount : '\u2014'}
                            </Text>
                        )}
                    </View>
            ...
            {isGlobal && (
                <FilterChipRow
                    bordered={false}
                    filters={activeTab === 'issues' ? [
                        { key: 'all', label: t('github.all') },
                        { key: 'assigned', label: t('github.assignedToMe') },
                        { key: 'created', label: t('github.createdByMe') },
                    ] : [
                        { key: 'all', label: t('github.all') },
                        { key: 'review-requested', label: t('github.reviewRequested') },
                        { key: 'created', label: t('github.createdByMe') },
                    ]}
                    value={activeTab === 'issues' ? issueScope : pullScope}
                    onChange={(value) => activeTab === 'issues'
                        ? setIssueScope(value as GithubIssueScope)
                        : setPullScope(value as GithubPullScope)}
                    style={styles.scopeFilters}
                />
            )}
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.groupped.background }]}>
            ...
            {(tokenExpired || activeResult.tokenExpired) && !isLoading ? (
                ...
                <FlatList
                    data={filteredIssues}
                    keyExtractor={(item) => `${item.repositoryFullName ?? selectedRepo}-${item.number}`}
                    renderItem={renderIssueItem}
                    ListHeaderComponent={listHeader}
                    contentInsetAdjustmentBehavior={Platform.OS === 'ios' ? 'automatic' : undefined}
                    refreshControl={<RefreshControl refreshing={isPullRefreshing} onRefresh={handleRefresh} tintColor={theme.colors.textSecondary} />}
                    ...
                    onEndReached={issueResult.hasMore ? issueResult.loadMore : undefined}
                    onEndReachedThreshold={0.5}
                    ListFooterComponent={issueResult.loadingMore ? (
                ...
                <FlatList
                    ...
                    refreshControl={<RefreshControl refreshing={isPullRefreshing} onRefresh={handleRefresh} tintColor={theme.colors.textSecondary} />}
                    contentContainerStyle={[styles.listContent, { maxWidth: layout.maxWidth, alignSelf: 'center', width: '100%', paddingBottom: tabBottomPadding }]}
                    onEndReached={pullResult.hasMore ? pullResult.loadMore : undefined}
                    onEndReachedThreshold={0.5}
                    ListFooterComponent={pullResult.loadingMore ? (
                        <ActivityIndicator style={{ paddingVertical: 16 }} color={theme.colors.textSecondary} />
                    ) : null}
                    ListEmptyComponent={renderEmptyState()}
                />
            ...
            >
                ...
                <View style={styles.sheetSearchContainer}>
                    <View style={[styles.sheetSearchBox, { backgroundColor: theme.colors.surface }]}>
                ...
                <Pressable
                    style={[styles.repoItem, selectedRepo === null && { backgroundColor: theme.colors.surfaceHigh }]}
                    onPress={() => handleSelectRepo(null)}
                >
                    <View style={[styles.repoAvatar, { backgroundColor: theme.colors.surfaceHigh }]}>
                        <Ionicons name="git-network-outline" size={17} color={theme.colors.text} />
                    </View>
                    <View style={styles.repoItemContent}>
                        <Text style={[styles.repoItemName, { color: theme.colors.text }]}>{t('github.allRepos')}</Text>
                        <Text style={[styles.repoItemMetaText, { color: theme.colors.textSecondary }]}>{t('github.allReposDescription')}</Text>
                    </View>
                    {selectedRepo === null && <Ionicons name="checkmark" size={18} color={theme.colors.repo.stateOpen} />}
                </Pressable>
