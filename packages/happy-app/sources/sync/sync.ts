        ...
        try {
            ...
            if (dooPushRegistration) {
                ...
                const cleanupResults = await cleanupSupersededPushTokens({
                    ...
                    deleteToken: (token, cleanupSignal) => deletePushToken(this.credentials, token, cleanupSignal),
                });
                ...
                try {
                    ...
                    const expoToken = await getExpoCompatibilityToken(() =>
                        Notifications.getExpoPushTokenAsync({
                            projectId: Constants?.expoConfig?.extra?.eas?.projectId
                                ?? Constants?.easConfig?.projectId,
                        })
                    );
                    if (expoToken === null || !startMutation()) {
                        return;
                    }
                    await registerPushToken(
                        this.credentials,
