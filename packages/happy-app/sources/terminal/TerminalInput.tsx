...
import { resolveNativeTerminalKey, type NativeTerminalKey } from "./terminalKeyEvents";
  ...
  function TerminalInput({ isKeyboardVisible, onFocus, onInput, onTerminalKey, style }, ref) {
    ...
    const showNativeKeyboard = useCallback(() => {
      ...
      const focusRequest = resolveTerminalInputFocusRequest({
        isInputFocused: isFocusedRef.current,
        isKeyboardVisible,
      });
      ...
      // The keyboard is up but this input is not focused, so focus alone can be
      // swallowed. Drop it first and re-take it on the next frame.
      input.blur();
    ...
    useImperativeHandle(
      ...
      [blurNativeInput, showNativeKeyboard],
    );
