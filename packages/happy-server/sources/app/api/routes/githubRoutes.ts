...
import { buildRepositoryQueries, listWorkRepositories, REPOSITORY_AFFILIATIONS, searchRepositoryWorkItems } from '@/app/github/githubWorkItems';
import { createWorkMetrics, githubWorkCache, githubWorkCacheScope } from '@/app/github/githubWorkCache';
...
type WorkItemType = 'issues' | 'pulls';
type WorkItemState = 'all' | 'open' | 'closed' | 'merged';
type WorkItemScope = 'all' | 'assigned' | 'review-requested' | 'created';

export function buildWorkItemSearchQuery(options: {
    type: WorkItemType;
    state: WorkItemState;
    scope: WorkItemScope;
    repository?: string;
    ...
    if (options.state === 'merged') qualifiers.push('is:merged');
    ...
    if (options.repository) qualifiers.push(`repo:${options.repository}`);
...
export function githubRoutes(app: Fastify) {
    ...
    // Restrict cross-repository work to the same affiliations as the repository picker.
    app.get('/v1/github/work-items', {
        preHandler: app.authenticate,
        schema: {
            querystring: z.object({
                type: z.enum(['issues', 'pulls']),
                state: z.enum(['all', 'open', 'closed', 'merged']).default('open'),
                scope: z.enum(['all', 'assigned', 'review-requested', 'created']).default('all'),
            ...
            response: {
                200: z.union([
                    paginatedSchema(RepoIssueSchema),
                    paginatedSchema(RepoPRSchema),
                ]),
        ...
        try {
            ...
            if (typeof auth.token !== 'string' || !auth.token) throw new GitHubNotConnectedError();
            const context = { scope: githubWorkCacheScope(request.userId, auth.token), metrics };
            cacheScope = context.scope;
            const { type, state, scope, repository, search, cursor, limit, refresh } = request.query;
            if (refresh === 'true' && !cursor) githubWorkCache.invalidate(context.scope);
            const repositoryStarted = performance.now();
            const repositories = await listWorkRepositories(octokit, context);
            repositoryMs = performance.now() - repositoryStarted;
            const allowedRepositories = repository
                ? repositories.filter((name) => name.toLowerCase() === repository.toLowerCase())
                : repositories;
            const q = buildWorkItemSearchQuery({ type, state, scope, search });
            const queries = buildRepositoryQueries(q, allowedRepositories);
            repositoryCount = allowedRepositories.length;
            groupCount = queries.length;
            const data = await searchRepositoryWorkItems(octokit, queries, limit, cursor, context);
            success = true;
            if (type === 'issues') {
                return reply.send({ ...data, items: data.items.map(mapIssue) });
            }
            return reply.send({ ...data, items: data.items.map(mapPR) });
        } catch (error) {
            if (cacheScope) githubWorkCache.invalidate(cacheScope);
            return handleGitHubError(error, reply);
        } finally {
            log({ source: 'github-work-items', type: request.query.type, pagination: Boolean(request.query.cursor),
                success, durationMs: Math.round(performance.now() - started), repositoryMs: Math.round(repositoryMs),
                repositoryCount, groupCount, ...metrics }, 'GitHub work items request');
        }
    });
            ...
            if (search && search.trim()) {
                ...
                const gqlQuery = `
                    query($cursor: String, $orderBy: RepositoryOrder!, $first: Int!) {
                        viewer {
                            repositories(first: $first, after: $cursor, orderBy: $orderBy, affiliations: ${REPOSITORY_AFFILIATIONS}) {
                                nodes {
                                    nameWithOwner
                                    name
                                    owner { login avatarUrl }
