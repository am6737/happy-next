...
| 聊天体验 | 图片附件、分页、蓝点、紧凑视图、会话搜索、下拉刷新、离线缓存对话缩略图导航、虚拟化 Web 对话列表(即时居中跳转)、上下文用量提示、可调侧边栏 |
| 会话管理 | 按机器分会话标签页、设备和 Agent 筛选、热升级、元数据缓存、智能新建会话机器默认值 |
...
- **虚拟化 Web 对话列表**：Web 聊天列表重写为模型驱动的虚拟化列表——跳转到某条消息时即时居中（已在目标处则给出轻微抖动反馈），历史随滚动按需加载
